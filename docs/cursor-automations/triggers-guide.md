...
| Issue commented | `@cursor plan` / `@cursor fix plan` / `@cursor fix` / `@cursor reject` / `OK` |
...
| `.github/workflows/agent-trigger-issue-followup.yml` | `OK` / `@cursor fix plan` / `@cursor fix` / `@cursor reject` | 難易度別分岐 |
